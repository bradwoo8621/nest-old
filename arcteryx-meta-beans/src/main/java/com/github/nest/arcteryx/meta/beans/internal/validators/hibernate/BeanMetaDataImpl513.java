 ...
 * @author brad.wu
 */
public class BeanMetaDataImpl513<T> implements BeanMetaData<T> {
	...
	/**
	 * Set of all constraints for this bean type (defined on any implemented
	 * interfaces or super types)
	 */
	private final Set<MetaConstraint<?>> allMetaConstraints;

	/**
	 * Set of all constraints which are directly defined on the bean or any of
	...
	/**
	 * The class hierarchy for this class starting with the class itself going
	 * up the inheritance chain. Interfaces are not included.
	 */
	...
	private Set<MetaConstraint<?>> getClassLevelConstraints(Set<MetaConstraint<?>> constraints) {
		Set<MetaConstraint<?>> classLevelConstraints = partition(constraints, byElementType()).get(ElementType.TYPE);
	...
	private List<Class<?>> getValidDefaultGroupSequence(List<Class<?>> groupSequence) {
		...
		if (log.isTraceEnabled()) {
			log.tracef("Members of the default group sequence for bean %s are: %s.", beanClass.getName(),
	...
	public static class BeanMetaDataBuilder<T> {
		...
		public void add(BeanConfiguration<? super T> configuration) {
			if (configuration.getBeanClass().equals(beanClass)) {
				if (configuration.getDefaultGroupSequence() != null
						&& (sequenceSource == null || configuration.getSource().getPriority() >= sequenceSource
								.getPriority())) {
				...
				if (configuration.getDefaultGroupSequenceProvider() != null
						&& (providerSource == null || configuration.getSource().getPriority() >= providerSource
								.getPriority())) {
		...
		private void addMetaDataToBuilder(ConstrainedElement constrainableElement, Set<BuilderDelegate> builders) {
			...
			builders.add(new BuilderDelegate(beanClass, constrainableElement, constraintHelper, executableHelper));
		}

		public BeanMetaDataImpl513<T> build() {
			...
			return new BeanMetaDataImpl513<T>(beanClass, defaultGroupSequence, defaultGroupSequenceProvider,
					aggregatedElements);
		}
	}
		...
		public boolean add(ConstrainedElement constrainedElement) {
			...
			if (propertyBuilder != null && propertyBuilder.accepts(constrainedElement)) {
				...
				if (!added && constrainedElement.getKind() == ConstrainedElementKind.METHOD && methodBuilder == null) {
					...
					methodBuilder = new ExecutableMetaData.Builder(beanClass, constrainedMethod, constraintHelper,
							executableHelper);
				}
